...
const createProductItemPopupTemplate = (flower) => {
  const {title, price, description, isAdding, isDisabled } = flower;

  let buttonText = !isAdding ? 'отложить' : 'отложено';

  if(isDisabled) {
    buttonText = !isAdding ? 'откладываем...' : 'возвращаем...';
  }
...
export default class ProductItemPopupView extends AbstractStatefulView {
  constructor(flower, isAdding) {
    super();
    this._state = {...flower, isAdding, isDisabled: false};
  }
  get template() {
