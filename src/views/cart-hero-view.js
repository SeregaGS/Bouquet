...
export default class CartHeroView extends AbstractView {
  ...
  handlerCartVisible = (callback) => {
    this._callback.handlerCartVisible = callback;
    this.element.querySelector('.btn-close').addEventListener('click', this.#handlerCartVisible);
  }
  #handlerCartVisible = (evt) => {
    evt.preventDefault();
    this._callback.handlerCartVisible();
  }
